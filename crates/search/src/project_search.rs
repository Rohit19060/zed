...
pub mod tests {
    ...
    use crate::buffer_search::FocusEditor;
    ...
    async fn test_buffer_search_query_reused(cx: &mut TestAppContext) {
        ...
        pane.update(&mut cx, |pane, cx| {
            pane.toolbar().update(cx, |toolbar, cx| {
                toolbar.add_item(search_bar.clone(), cx);
            })
        });

        search_bar
            .update(&mut cx, |search_bar, cx| {
                search_bar.search("search bar query", None, cx)
            })
            .await
            .unwrap();
        search_bar.update(&mut cx, |search_bar, cx| {
            search_bar.focus_editor(&FocusEditor, cx);
        });

        let project_search = cx.new_model(|cx| ProjectSearch::new(project, cx));
        workspace.update(&mut cx, |workspace, cx| {
            ProjectSearchView::new_search(workspace, &workspace::NewSearch, cx)
        });
        cx.run_until_parked();

        project_search.update(&mut cx, |project_search, _| {
            dbg!(&project_search.active_query);
        });
    }
